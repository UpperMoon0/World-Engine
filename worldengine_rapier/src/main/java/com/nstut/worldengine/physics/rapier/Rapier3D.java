...
/**
 * Java side of World Engine's Rapier bridge.
...
public final class Rapier3D {
    ...
    private static final String LIB_ZIP_NAME = "worldengine_rapier_binaries.zip.l4z";
    private static final String LIB_NAME = "worldengine_rapier";
    ...
    private static Path resolveNativeDir() {
        ...
        if (gameDir != null) {
            final Path gameDirRelativeDir = gameDir.resolve(".worldengine").resolve("natives").normalize();
            Sable.LOGGER.info("Using World Engine native directory {}", gameDirRelativeDir.toAbsolutePath());
            return gameDirRelativeDir;
        }

        final Path fallbackDir = Paths.get(System.getProperty("user.home", System.getProperty("user.dir")), ".worldengine", "natives");
        Sable.LOGGER.info("Using fallback World Engine native directory {}", fallbackDir.toAbsolutePath());
        return fallbackDir;
    }
    ...
    private static void loadLibrary() {
        final String developmentNative = System.getProperty("worldengine.rapier.nativePath");
        if (developmentNative != null && !developmentNative.isBlank()) {
            final Path nativePath = Paths.get(developmentNative).toAbsolutePath().normalize();
            Sable.LOGGER.warn("Loading Rapier from explicit development native path {}", nativePath);
    ...
    public static long getSceneHandle(final ServerLevel level) {
        ...
        return rapierPipeline.getDefaultSceneHandle();
    }

    @ApiStatus.Internal
    public static long getConstraintSceneHandle(final ServerLevel level,
        ...
        return rapierPipeline.prepareConstraintScene(bodyA, bodyB);
    }

    @ApiStatus.Internal
    public static native long createUniverse(int dimensionId);

    @ApiStatus.Internal
    public static native void destroyUniverse(long universeHandle);

    static native long initialize(final long universeHandle, double gravityX, double gravityY, double gravityZ, double universalDrag,
                                  double originX, double originY, double originZ);

    @ApiStatus.Internal
    public static native void flushUniverseCommands(final long universeHandle);

    @ApiStatus.Internal
    public static native void tickUniverse(final long universeHandle, long absoluteTick, double timeStep, double gx, double gy, double gz);

    @ApiStatus.Internal
    public static native int writeMaterializationRequests(final long universeHandle, java.nio.ByteBuffer buffer, int maxRequests);
    ...
    @ApiStatus.Internal
    public static native void applyImpulseUniverse(final long universeHandle, int id, double x, double y, double z, double fx, double fy, double fz);

    @ApiStatus.Internal
    public static native void applyForceAndTorqueUniverse(final long universeHandle, int id, double fx, double fy, double fz, double tx, double ty, double tz, boolean wakeUp);

    @ApiStatus.Internal
    public static native void addLinearAngularVelocityUniverse(final long universeHandle, int id, double lx, double ly, double lz, double ax, double ay, double az, boolean wakeUp);

    @ApiStatus.Internal
    public static native void wakeUpUniverse(final long universeHandle, int id);

    @ApiStatus.Internal
    public static native void teleportUniverse(final long universeHandle, int id, double x, double y, double z, double qx, double qy, double qz, double qw);

    @ApiStatus.Internal
    public static native void removeUniverseBody(final long universeHandle, int id);

    @ApiStatus.Internal
    public static native void setUniverseBodyStats(final long universeHandle, int id, double mass, double[] centerOfMass, double[] inertiaTensor, int minX, int minY, int minZ, int maxX, int maxY, int maxZ);

    @ApiStatus.Internal
    public static native void addUniverseSubLevelChunk(final long universeHandle, int id, int x, int y, int z, int[] chunk);

    @ApiStatus.Internal
    public static native void removeUniverseSubLevelChunk(final long universeHandle, int id, int x, int y, int z);

    @ApiStatus.Internal
    public static native void addWorldTerrainChunk(final long universeHandle, int x, int y, int z);

    @ApiStatus.Internal
    public static native void removeWorldTerrainChunk(final long universeHandle, int x, int y, int z);

    @ApiStatus.Internal
    public static native int writeUniverseDirtyPoses(final long universeHandle, java.nio.ByteBuffer buffer, int maxBodies);

    @ApiStatus.Internal
    public static native void changeUniverseSubLevelBlock(final long universeHandle, int id, int x, int y, int z, int packedState);

    @ApiStatus.Internal
    public static native void getUniversePose(final long universeHandle, int id, double[] outPose);

    @ApiStatus.Internal
    public static native void getUniverseLinearVelocity(final long universeHandle, int id, double[] outVel);

    @ApiStatus.Internal
    public static native void getUniverseAngularVelocity(final long universeHandle, int id, double[] outVel);

    @ApiStatus.Internal
    static native boolean migrateBody(long sourceSceneHandle, long destinationSceneHandle, int id);

    @ApiStatus.Internal
    public static native void rebaseRegionOrigin(final long sceneHandle, double x, double y, double z);

    @ApiStatus.Internal
    static native void tick(final long sceneHandle, double timeStep);

    @ApiStatus.Internal
    static native void step(final long sceneHandle, double timeStep, int elapsedTicks);

    @ApiStatus.Internal
    static native boolean hasActiveBodies(final long sceneHandle);

    @ApiStatus.Internal
    static native long ticksUntilNextScheduledBody(final long sceneHandle);

    @ApiStatus.Internal
    static native boolean canStepInParallel(final long sceneHandle);

    @ApiStatus.Internal
    public static native void processCommands(final long sceneHandle, java.nio.ByteBuffer buffer, int length);
    ...
    @ApiStatus.Internal
    static native void registerUniverseBody(final long universeHandle, int id, double[] pose);

    @ApiStatus.Internal
    public static native void materializeBody(final long universeHandle, int id, final long regionHandle);

    @ApiStatus.Internal
    public static native void evictBody(final long universeHandle, int id, final long regionHandle);
    ...
    @ApiStatus.Internal
    public static native boolean mergeScenes(final long sourceHandle, final long destinationHandle);
    ...
    /**
     * Bulk reads the pose of all active objects into a native Direct ByteBuffer.
     * @param sceneHandle The pointer pointing to the current Rapier simulation
     * @param buffer The direct ByteBuffer to fill (capacity should be big enough to store 60 bytes per active body)
     * @param maxBodies The maximum number of bodies the buffer can hold
     * @return the number of bodies written, or the negative required body
     * count when the supplied buffer is too small
     */
    @ApiStatus.Internal
    public static native int writeActivePoses(final long sceneHandle, java.nio.ByteBuffer buffer, int maxBodies);
     ...
     */
    @ApiStatus.Internal
    static native void removeChunk(final long sceneHandle, int x, int y, int z, boolean global, int id);
     ...
     */
    @ApiStatus.Internal
    public static native void changeWorldBlock(final long sceneHandle, int x, int y, int z, int block);

    @ApiStatus.Internal
    public static native void changeSubLevelBlock(final long sceneHandle, int x, int y, int z, int block, int subLevelId);
    ...
    @ApiStatus.Internal
    public static native void wakeUpRegionObject(final long sceneHandle, int id);
     ...
     */
    @ApiStatus.Internal
    public static native void wakeUpObject(final long universeHandle, int id);
     ...
     */
    @ApiStatus.Internal
    public static native void addLinearAngularVelocities(final long universeHandle, int bodyId, double linearX, double linearY, double linearZ, double angularX, double angularY, double angularZ, final boolean wakeUp);
     ...
     */
    @ApiStatus.Internal
    static native void applyForce(final long universeHandle, final int bodyID, final double x, final double y, final double z, final double fx, final double fy, final double fz, final boolean wakeUp);
     ...
     */
    @ApiStatus.Internal
    static native void applyForceAndTorque(final long universeHandle, final int bodyID, final double fx, final double fy, final double fz, final double tx, final double ty, final double tz, final boolean wakeUp);
     ...
     */
    @ApiStatus.Internal
    static native void setKinematicContraptionTransform(final long universeHandle, int id, double[] centerOfMass, double[] pose, double[] velocities);
    ...
    @ApiStatus.Internal
    static native void configFrequencyAndDamping(long sceneHandle,
            double contactNaturalFrequency,
            double contactDampingRatio);

    @ApiStatus.Internal
    static native void configSolverIterations(long sceneHandle, int solverIterations, int pgsIterations, int stabilizationIterations);
