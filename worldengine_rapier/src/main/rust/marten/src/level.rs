...
pub struct ChunkSection {
    ...
    pub serial_callback_blocks: u16,
}
...
impl ChunkSection {
    ...
    pub fn new(blocks: Vec<BlockState>) -> Self {
        ...
        Self {
            blocks,
            serial_callback_blocks: 0,
        }
    }

    pub fn with_serial_step(blocks: Vec<BlockState>, serial_callback_blocks: u16) -> Self {
        if blocks.len() != (CHUNK_SIZE as usize).pow(3) {
            panic!("Invalid block count: {}", blocks.len());
        }
    ...
    pub fn set_block(&mut self, x: i32, y: i32, z: i32, state: BlockState) -> BlockState {
