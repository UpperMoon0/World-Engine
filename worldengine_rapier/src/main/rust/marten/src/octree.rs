...
impl SubLevelOctree {
    ...
    /// Doubles the tree while retaining all existing nodes. `old_octant`
    /// selects which child of the new root contains the old tree.
    pub fn grow(&mut self, old_octant: i32) {
        assert!((0..8).contains(&old_octant));
        let old_root = self.buffer[0];
        let children = self.allocate_branch();
        for child in 0..8 {
            self.buffer[(children + child) as usize] = 0;
        }
        self.buffer[(children + old_octant) as usize] = old_root;
        self.buffer[0] = children;
        self.log_size += 1;
    }
...
#[cfg(test)]
mod tests {
    use super::SubLevelOctree;

    #[test]
    fn growing_root_preserves_existing_leaves() {
        let mut positive = SubLevelOctree::new(2);
        positive.insert(1, 2, 3, 7);
        positive.grow(0);
        assert_eq!(positive.query(1, 2, 3, 0), 7);

        let mut negative = SubLevelOctree::new(2);
        negative.insert(1, 2, 3, 9);
        negative.grow(7);
        assert_eq!(negative.query(5, 6, 7, 0), 9);
    }
}
