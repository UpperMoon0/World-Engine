...
use marten::level::OctreeChunkSection;
...
pub fn find_collision_pairs(
    ...
    cutoff: usize,
    liquid: bool,
    world_octree_chunks: &HashMap<i64, OctreeChunkSection>,
) -> Vec<(IVec3, IVec3)> {
    struct StackObject {
        index: u32,
        depth: u32,
    ...
    let local_bounds_min = sable_body.octree_origin.unwrap();
...
fn get_overlapping_nodes(
    ...
    if let Some(other_handle) = other_handle {
        let other_min = other_handle.octree_origin.unwrap();
    ...
    for ox in min_octree_pos.x..=max_octree_pos.x {
        for oy in min_octree_pos.y..=max_octree_pos.y {
            for oz in min_octree_pos.z..=max_octree_pos.z {
                let chunk = world_octree_chunks.get(&pack_section_pos(ox, oy, oz));
