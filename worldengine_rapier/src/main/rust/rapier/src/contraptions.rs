...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_createKinematicContraption<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
    ...
    with_handle(handle, |scene| {
        ...
        let mount_rigid_body = if should_be_static {
            let new_body = sim_data.rigid_body_set.insert(
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_setKinematicContraptionTransform<
    'local,
>(
    env: JNIEnv<'local>,
    _class: JClass<'local>,
    ...
    crate::with_universe(handle, |universe| {
        if let Some(body) = universe.universe_bodies.get_mut(&(id as crate::scene::LevelColliderID)) {
            body.command_queue.push(crate::scene::UniverseCommand::SetKinematicContraptionTransform {
                center_of_mass: center_of_mass_arr,
                pose: pose_arr,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_addKinematicContraptionChunkSection<
    'local,
>(
    env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_removeKinematicContraption<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
