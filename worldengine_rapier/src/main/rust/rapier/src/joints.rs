...
use crate::scene::{
    LevelColliderID, PhysicsScene, SableSceneData, SimulationSceneData, SimulationTier,
};
use crate::{instantiate_rapier_body, with_handle};
use jni::JNIEnv;
use jni::objects::{JClass, JDoubleArray};
use jni::sys::{jboolean, jbyte, jdouble, jint, jlong};
use marten::Real;
use rapier3d::dynamics::{
    GenericJoint, GenericJointBuilder, JointAxesMask, JointAxis, RevoluteJointBuilder,
    RigidBodyHandle, SpringCoefficients,
};
...
static NEXT_SABLE_JOINT_ID: AtomicI64 = AtomicI64::new(1);

fn next_sable_joint_handle() -> SableJointHandle {
    NEXT_SABLE_JOINT_ID.fetch_add(1, Ordering::Relaxed)
}

fn prepare_joint_body(
    ...
    if !sable_data.rigid_bodies.contains_key(&id) {
        crate::instantiate_rapier_body(sim_data, sable_data, &mut *universe, world_origin, id);
    }
    if let Some(ubody) = universe.universe_bodies.get_mut(&id) {
        ubody.simulation_tier = SimulationTier::Critical;
    }
    let tick = universe.current_tick + 5;
    universe.schedule_body(id, tick);
    sable_data.rigid_bodies[&id]
}

#[derive(Clone)]
pub(crate) struct SubLevelJoint {
...
pub struct SableJointSet {
    ...
    assemblies_dirty: bool,
    assembly_members: HashSet<LevelColliderID>,
}
...
impl SableJointSet {
    ...
    pub fn new() -> Self {
        Self {
            joints: HashMap::new(),
            assemblies_dirty: false,
            assembly_members: HashSet::new(),
        }
    }
    pub(crate) fn take_for_scene_merge(
        &mut self,
        sim: &SimulationSceneData,
    ) -> Vec<(SableJointHandle, SubLevelJoint, GenericJoint)> {
        let mut result = Vec::with_capacity(self.joints.len());
        for (logical_handle, joint) in self.joints.drain() {
            if let Some(rapier_joint) = sim.impulse_joint_set.get(joint.handle) {
                result.push((logical_handle, joint, rapier_joint.data.clone()));
            }
        }
        self.assembly_members.clear();
        self.assemblies_dirty = false;
        result
    }

    pub(crate) fn restore_after_scene_merge(
        &mut self,
        migrating: Vec<(SableJointHandle, SubLevelJoint, GenericJoint)>,
        sim: &mut SimulationSceneData,
        rigid_bodies: &HashMap<LevelColliderID, RigidBodyHandle>,
        ground: RigidBodyHandle,
    ) {
        for (logical_handle, mut joint, data) in migrating {
            let body_a = joint.id_a.map(|id| rigid_bodies[&id]).unwrap_or(ground);
            let body_b = joint.id_b.map(|id| rigid_bodies[&id]).unwrap_or(ground);
            joint.handle = sim.impulse_joint_set.insert(body_a, body_b, data, true);
            self.joints.insert(logical_handle, joint);
        }
        self.assemblies_dirty = true;
    }
}
...
pub fn tick(scene: &PhysicsScene) {
    ...
    // filter the joints
    let previous_joint_count = sable_data.joint_set.joints.len();
    sable_data
        .joint_set
        .joints
        .retain(|_handle, joint| sim.impulse_joint_set.contains(joint.handle));
    if sable_data.joint_set.joints.len() != previous_joint_count {
        sable_data.joint_set.assemblies_dirty = true;
    }

    if sable_data.joint_set.assemblies_dirty {
        rebuild_fixed_assemblies(&mut sable_data, &mut universe);
        sable_data.joint_set.assemblies_dirty = false;
    }
...
fn rebuild_fixed_assemblies(sable_data: &mut SableSceneData, universe: &mut crate::scene::DimensionUniverse) {
    for id in std::mem::take(&mut sable_data.joint_set.assembly_members) {
        if let Some(body) = universe.universe_bodies.get_mut(&id) {
            body.assembly_root = id;
            body.assembly_size = 1;
        }
    }

    let mut parent: HashMap<LevelColliderID, LevelColliderID> = HashMap::new();

    fn find(
        parent: &mut HashMap<LevelColliderID, LevelColliderID>,
        mut id: LevelColliderID,
    ) -> LevelColliderID {
        while parent[&id] != id {
            let next = parent[&id];
            let grandparent = parent[&next];
    ...
    for joint in sable_data
        .joint_set
        .joints
        .values()
        .filter(|joint| joint.assembly_link)
    {
        let (Some(a), Some(b)) = (joint.id_a, joint.id_b) else {
            continue;
        };
        if !universe.universe_bodies.contains_key(&a)
            || !universe.universe_bodies.contains_key(&b)
        {
            continue;
        }
    ...
    let ids: Vec<_> = parent.keys().copied().collect();
    let mut sizes: HashMap<LevelColliderID, u32> = HashMap::new();
    for id in &ids {
    ...
    for id in ids {
        ...
        body.assembly_root = root;
        body.assembly_size = sizes[&root];
        sable_data.joint_set.assembly_members.insert(id);
    }
}
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_setConstraintMotor<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_setConstraintLimit<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_lockConstraintAxes<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_isConstraintValid<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_getConstraintImpulses<
    'local,
>(
    env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_setConstraintContactsEnabled<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_removeConstraint<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_addRotaryConstraint<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
    ...
    with_handle(handle, |scene| {
        ...
        let rb_a = if id_a == -1 {
            ...
            prepare_joint_body(&mut sim_data, &mut sable_data, &mut *universe, *scene.world_origin.read().unwrap(), id_a as LevelColliderID)
        };

        let rb_b = if id_b == -1 {
            scene.ground_handle.unwrap()
        } else {
            prepare_joint_body(&mut sim_data, &mut sable_data, &mut *universe, *scene.world_origin.read().unwrap(), id_b as LevelColliderID)
        };
        ...
        let handle_long: SableJointHandle = next_sable_joint_handle();
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_addFixedConstraint<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_addFreeConstraint<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_addGenericConstraint<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_com_nstut_worldengine_physics_rapier_Rapier3D_setConstraintFrame<
    'local,
>(
    _env: JNIEnv<'local>,
    _class: JClass<'local>,
